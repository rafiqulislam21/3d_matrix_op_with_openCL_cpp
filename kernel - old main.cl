__kernel void ThreeDimArray(__global float *const A) {
  ...
  //barrier doesn't work for global memory, we should write another kernel for function''''''
  ...
  // // iteration part
  // if(i >= 1 && i <= row-1){
  //   const int idk0i1 = 0 * row * column + (i+1) * column + j; // indexes for
  //   k=0 and i+1 const int idk2i1 = 2 * row * column + (i-1) * column + j; //
  //   indexes for k=2 and i-1 for (int t = 0; t < 24; t++) {
  //     // A_seq[1][i][j] = A_seq[1][i][j] + (1 / (sqrt(A_seq[0][i + 1][j] +
  //     A_seq[2][i - 1][j]))); A[idk1] = A[idk1] + (float)(1 /
  //     (native_sqrt(A[idk0i1] + A[idk2i1])));
  //   }
  // }
  //------------accurate but slower---------------------------------------
};
