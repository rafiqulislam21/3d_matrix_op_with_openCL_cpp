__kernel void ThreeDimArray( __global float *const A_input, __global float *const A) {
   ...
   if (i >= 1 && i <= row - 1) {
    if (k == 1) {
      ...
      for (int t = 0; t < 24; t++) {
        // A_seq[1][i][j] = A_seq[1][i][j] + (1 / (sqrt(A_seq[0][i + 1][j] +
        // A_seq[2][i - 1][j])));
        A_input[idx] = A_input[idx] + (float)(1 / (native_sqrt(A_input[idk0i1] + A_input[idk2i1])));
      }
    }
  }
  //------------accurate but slower---------------------------------------
